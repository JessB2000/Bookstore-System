...
public class Professor implements IUsuario, Observer {
	...
	public void pegarLivroEmprestado(ILivro livro) throws Exception {
		...
		if (listaEmprestimoAtivo.size() >= LIMITE_EMPRESTIMOP) {
		...
		if (reserva == SEM_RESERVA) { 
			...
			EmprestimoLivro emprestimo  = new EmprestimoLivro(this, livro, LocalDate.now(), LocalDate.now().plusDays(PRAZO_DEVOLUCAOP));
	...
	public void reservarLivro(ILivro livro) throws Exception {
		if (listaReservaAtiva.size() >= LIMITE_RESERVAP) {
